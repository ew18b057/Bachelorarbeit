...
// Battery Pin and amber Led declaration
var batteryState = false;
var pinBattery = tessel.port.A.pin[5];
var ledAmber = tessel.led[1];

// Alarm pin defined to pulldown for a clean GND signal of 0.0V
pinAlarm.pull("pulldown");

// Battery pin defined to pulldown for a clean GND signal of 0.0V
pinBattery.pull("pulldown");
...
setInterval(readoutBatteryState, 100);

function readoutBatteryState() {
    pinBattery.read((error, value) => {
        if (value) {
            console.log("PIN A5 read: connected");
            ledAmber.off();
            batteryState = true;
        } else {
            console.log("PIN A2 read: not connected");
            ledAmber.on();
            batteryState = false;
        }
    });
